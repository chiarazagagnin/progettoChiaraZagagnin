    ...
    >
      ...
      <md-card md-with-hover>
        <md-card-header>
          ...
          <div v-if="type == 'album'" class="md-subhead card-text">{{like.artists[0].name}}</div>
          <div v-if="type == 'track'" class="md-subhead card-text">{{like.album.artists[0].name}}</div>
        </md-card-header>
...
export default {
  ...
  methods: {
    ...
    disLike(id) {
      ...
      DataService.getDocs(this.type).then(data => {
        this.dbdocs = data;
        let index = this.ids.indexOf(id);
        let doc = this.dbdocs[index];
        DataService.removeLike(this.type, doc);
        setTimeout(() => {
          this.load();
          this.loading = false;
        }, 1000);
